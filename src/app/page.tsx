  ...
  return (
    <main className="max-w-2xl mx-auto px-4 py-12">
      <div className="mb-8 text-center">
        ...
        <p className="text-gray-500 text-sm">Şehirleri gir, AI destekli planını al.</p>
      </div>
      <div className="bg-white rounded-2xl shadow p-6 space-y-6">
        <div>
          <label className="block text-sm font-medium mb-1">Gezi Başlığı</label>
        ...
        <div>
          <label className="block text-sm font-medium mb-2">Şehirler (sırasıyla)</label>
          <div className="space-y-2">
            {cities.map((city, i) => (
              <div key={i} className="flex gap-2 items-center">
                <span className="text-xs text-gray-400 w-5">{i+1}.</span>
                <input className="flex-1 border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400"
                  placeholder="Şehir adı" value={city.name} onChange={e => updateCity(i, "name", e.target.value)} />
                <input type="number" min={1} max={14}
                  className="w-20 border rounded-lg px-3 py-2 text-sm text-center focus:outline-none focus:ring-2 focus:ring-indigo-400"
                  value={city.nights} onChange={e => updateCity(i, "nights", parseInt(e.target.value)||1)} />
                <span className="text-xs text-gray-400">gece</span>
                {cities.length > 2 && (
                  <button onClick={() => removeCity(i)} className="text-red-400 hover:text-red-600 text-xl">×</button>
                )}
              </div>
            ))}
          </div>
          <button onClick={addCity} className="mt-3 text-sm text-indigo-600 hover:text-indigo-800 font-medium">+ Şehir Ekle</button>
        </div>
        <div>
          ...
          <div className="flex gap-2 flex-wrap">
            {[["mixed","🔀 Karma"],["train","🚆 Tren"],["car","🚗 Araç"],["flight","✈️ Uçuş"]].map(([v,l]) => (
              <button key={v} onClick={() => setMode(v)}
                className={`px-4 py-2 rounded-full text-sm border transition ${mode===v?"bg-indigo-600 text-white border-indigo-600":"bg-white text-gray-600 border-gray-300"}`}>{l}</button>
            ))}
          </div>
        </div>
        <div>
          ...
          <div className="flex gap-2">
            {[["relaxed","🛋️ Sakin"],["medium","⚖️ Orta"],["intense","⚡ Yoğun"]].map(([v,l]) => (
              <button key={v} onClick={() => setPace(v)}
                className={`px-4 py-2 rounded-full text-sm border transition ${pace===v?"bg-indigo-600 text-white border-indigo-600":"bg-white text-gray-600 border-gray-300"}`}>{l}</button>
            ))}
          </div>
        </div>
